...
import {
  ...
  soundVolume as _soundVolume_from_index,

  setSoundVolume as _setSoundVolume_from_index,

  missionDescriptions as _missionDescriptions_from_index,

  missionExpRewards as _missionExpRewards_from_index,
...
import { soundVolume, setSoundVolume } from './src/audio/volume.js';

import { missionDescriptions, missionExpRewards } from "./src/missions/utils.js";
...
import { runImageCompressionMigration } from './scripts/migrationManager.js';
...
if (!Object.getOwnPropertyDescriptor(window, 'villageChief')) {
  ...
  Object.defineProperty(window, 'villageChief', {
    ...
    set: v => { 
      if (v && v.level === 1 && villageChief.level > 1) {
        console.error('🚨 ALERTA: villageChief siendo sobrescrito completamente!');
        console.trace('Stack trace del sobrescribir:');
      }
      villageChief = createProtectedProxy(v, 'villageChief'); 
    },
...
console.log('🔧 Variables globales inicializadas:', {
  villageChief: { level: villageChief.level, exp: villageChief.exp, name: villageChief.name },
  partner: { level: partner.level, exp: partner.exp, name: partner.name }
});

// Crear proxies para detectar cuando se sobrescriben las variables
const originalVillageChief = villageChief;
const originalPartner = partner;

// Función para crear un proxy que detecte cambios
function createProtectedProxy(obj, name) {
  return new Proxy(obj, {
    set(target, property, value) {
      if (property === 'level' && value === 1 && target.level > 1) {
        console.error(`🚨 ALERTA: ${name}.${property} siendo reseteado de ${target.level} a ${value}`);
        console.trace('Stack trace del reseteo:');
      }
      if (property === 'exp' && value === 0 && target.exp > 0) {
        console.error(`🚨 ALERTA: ${name}.${property} siendo reseteado de ${target.exp} a ${value}`);
        console.trace('Stack trace del reseteo:');
      }
...
// Proteger las variables con proxies
villageChief = createProtectedProxy(villageChief, 'villageChief');
partner = createProtectedProxy(partner, 'partner');

if (!Object.getOwnPropertyDescriptor(window, 'partner')) {
  ...
  Object.defineProperty(window, 'partner', {
    ...
    set: v => { 
      if (v && v.level === 1 && partner.level > 1) {
        console.error('🚨 ALERTA: partner siendo sobrescrito completamente!');
        console.trace('Stack trace del sobrescribir:');
      }
      partner = createProtectedProxy(v, 'partner'); 
    },
...
async function loadGame() {
  ...
  if (!data) {
    console.warn('⚠️ No se encontraron datos de guardado');
    return;
  }

  console.log('📁 Datos de guardado cargados:', {
    version: data.version,
    hasVillageChief: !!data.villageChief,
    hasPartner: !!data.partner,
    villageChiefLevel: data.villageChief?.level,
    partnerLevel: data.partner?.level
  });
  ...
  try {
    ...
    console.error('Error al cargar lastsave.json:', error);
  ...
  // Cargar datos de villageChief con validación mejorada
  if (data.villageChief) {
    villageChief = { ...villageChief, ...data.villageChief };
    console.log('✅ VillageChief cargado desde save.json:', {
      level: villageChief.level,
      exp: villageChief.exp,
      name: villageChief.name,
      familiars: villageChief.familiars?.length || 0,
      habilities: villageChief.habilities?.length || 0
    });
  } else {
    console.warn('⚠️ No se encontraron datos de villageChief en save.json');
  }
  ...
  // Cargar datos de partner con validación mejorada
  if (data.partner) {
    partner = { ...partner, ...data.partner };
    console.log('✅ Partner cargado desde save.json:', {
      level: partner.level,
      exp: partner.exp,
      name: partner.name,
      energia: partner.energia
    });
  } else {
    console.warn('⚠️ No se encontraron datos de partner en save.json');
  }
...
function finishTimer(timer) {
  ...
  // Forzar guardado inmediato después de completar misiones para evitar pérdida de datos
  console.log('💾 Guardando datos después de completar misión...');
  saveGame();
  
  // También programar guardado adicional como respaldo
  scheduleSaveGame();
...
export function saveGame() {
  ...
  if (!villageChief || typeof villageChief !== 'object') {
    console.error('❌ Error: villageChief no está definido o no es un objeto');
    return;
  }
  
  if (!partner || typeof partner !== 'object') {
    console.error('❌ Error: partner no está definido o no es un objeto');
    return;
  }

  console.log('💾 Guardando datos del juego:', {
    villageChief: {
      level: villageChief.level,
      exp: villageChief.exp,
      name: villageChief.name,
...
export function scheduleSaveGame() {
  console.log('📅 scheduleSaveGame llamado - Estado actual:', {
    villageChief: { level: villageChief?.level, exp: villageChief?.exp, name: villageChief?.name },
    partner: { level: partner?.level, exp: partner?.exp, name: partner?.name }
  });
  ...
  saveTimeout = setTimeout(() => {
    ...
    console.log('⏰ Ejecutando saveGame programado - Estado antes de guardar:', {
      villageChief: { level: villageChief?.level, exp: villageChief?.exp, name: villageChief?.name },
      partner: { level: partner?.level, exp: partner?.exp, name: partner?.name }
    });
...
function updateResourcesDisplay() {
  ...
  // Actualizar contadores adicionales en la card de Population
  const familiarsEl = document.getElementById("familiars-display");
  if (familiarsEl) {
    const unlockedFamiliars = villageChief.unlockedFamiliars || 0;
    familiarsEl.textContent = `Familiars: ${unlockedFamiliars}/100`;
  }

  const heroesTotalEl = document.getElementById("heroes-total-display");
  if (heroesTotalEl) heroesTotalEl.textContent = `Heroes: ${state.heroes.length}`;

  const villainsTotalEl = document.getElementById("villains-total-display");
  if (villainsTotalEl) villainsTotalEl.textContent = `Villains: ${villains.length}`;

  const petsTotalEl = document.getElementById("pets-total-display");
  if (petsTotalEl) {
    const totalPets = state.heroes.filter(h => h.pet).length;
    petsTotalEl.textContent = `Pets: ${totalPets}`;
  }

  // Actualizar estado de todos los botones en la tarjeta de Population
  const citizenBtnEl = document.getElementById("citizen-btn");
  if (citizenBtnEl) {
    citizenBtnEl.disabled = citizens >= state.terrain * 50 || state.money < 100;
    if (citizenBtnEl.disabled && state.money < 100) citizenBtnEl.title = "Not enough Gold";
  }

  const soldierBtnEl = document.getElementById("soldier-btn");
  if (soldierBtnEl) {
    soldierBtnEl.disabled = soldiers >= state.terrain * 50 || state.money < 200;
    if (soldierBtnEl.disabled && state.money < 200) soldierBtnEl.title = "Not enough Gold";
  }

  const eliteBtnEl = document.getElementById("elite-btn");
  if (eliteBtnEl) {
    eliteBtnEl.disabled = state.money < 1500;
    if (eliteBtnEl.disabled) eliteBtnEl.title = "Not enough Gold";
  }

  const specialCitizenBtnEl = document.getElementById("special-citizen-btn");
  if (specialCitizenBtnEl) {
    specialCitizenBtnEl.disabled = state.money < 300;
    if (specialCitizenBtnEl.disabled) specialCitizenBtnEl.title = "Not enough Gold";
  }

  const specialSoldierBtnEl = document.getElementById("special-soldier-btn");
  if (specialSoldierBtnEl) {
    specialSoldierBtnEl.disabled = state.money < 600;
    if (specialSoldierBtnEl.disabled) specialSoldierBtnEl.title = "Not enough Gold";
  }
  ...
  if (famBtn) {
    ...
    const familiarCost = 1500 * ((villageChief.unlockedFamiliars || unlockedFamiliars) + 1);

    const noGold = state.money < familiarCost;

    famBtn.disabled = noGold || (villageChief.unlockedFamiliars || unlockedFamiliars) >= FAMILIAR_COUNT;
...
function addMoveArrows(container, img, obj, key = 'avatarOffset', isBg = false, xKey = null) {
  // Navigation arrows and zoom controls removed as requested
  // Only apply initial positioning without any controls
  function apply() {
    const y = obj[key] ?? 50;
...
function chooseHeroesForTowerDefense(container, gamesSection) {
  ...
  title.textContent = 'Select 12 Heroes for Tower Defense';
  ...
  description.textContent = 'These heroes will represent your 12 towers in the game. Select strategically to maximize your defense.';
  ...
  heroesContainer.style.flexDirection = 'column';

  heroesContainer.style.gap = '15px';

  heroesContainer.style.marginBottom = '20px';

  

  const rows = [];

  const rowCount = 2; // 2 filas de 6 héroes
  ...
  for (let i = 0; i < 12; i++) {
    ...
    wrap.style.border = '2px solid #34495e';

    wrap.style.borderRadius = '10px';

    wrap.style.backgroundColor = 'rgba(52, 73, 94, 0.1)';
    ...
    sel.style.border = '1px solid #bdc3c7';

    sel.style.backgroundColor = '#2c3e50';

    sel.style.color = '#ecf0f1';

    sel.addEventListener('change', () => { 

      refreshTowerDefenseOptions(); 

      checkTowerDefenseReady(); 
    ...
    rows[Math.floor(i / 6)].appendChild(wrap); // 6 héroes por fila

  }

  

  modal.appendChild(heroesContainer);
  ...
  startBtn.textContent = '🎮 Start Tower Defense';
  ...
  cancel.style.padding = '12px 24px';
  ...
  startBtn.onclick = () => {
    ...
    const selectedHeroes = ids.map(id => state.heroMap.get(id));

    loadTowerDefense(container, selectedHeroes);
  ...
  appendOverlay(overlay, gamesSection);

  

  function refreshTowerDefenseOptions() {
    ...
    selects.forEach((sel, idx) => {
      ...
      state.heroes.forEach(h => {
        ...
        if (!already && !isBusy(h) && (h.energia ?? 0) >= 100) {

          const o = document.createElement('option');

          o.value = h.id;

          o.textContent = `${h.name} (Lv ${h.level || 1})`;
    ...
    refreshTowerDefenseAvatars();

  }

  

  function refreshTowerDefenseAvatars() {
    ...
    selects.forEach((sel, i) => {
      ...
      if (id) {
        ...
        if (!hero.avatar) {
          ...
          img.style.backgroundColor = '#34495e';
  ...
  function checkTowerDefenseReady() {
...
function loadTowerDefense(container, selectedHeroes) {
  ...
  container.dataset.game = "TowerDefense";
  ...
  iframe.src = "src/OtherMinigames/TowerDefense.html";
  ...
  // Esperar a que el iframe se cargue para pasar los datos de los héroes

  iframe.onload = () => {

    try {

      // Pasar los datos de los héroes al iframe

      const heroData = selectedHeroes.map(hero => ({
        ...
        avatarOffset: hero.avatarOffset || 50
      ...
      console.error('Error al pasar datos de héroes a Tower Defense:', error);
...
window.addEventListener('message', (e) => {
  ...
  } else if (d.type === 'projectsLevelUp' && d.abilityName && Number.isFinite(d.delta)) {

    // Sincroniza nivel de P_Ability recibido desde Projects.html (producción)

    const list = villageChief?.partnerAbilities || partner?.abilities || [];

    const ab = list.find(a => a?.name === d.abilityName || a?.label === d.abilityName);

    if (ab) {

      ab.level = (ab.level ?? ab.lvl ?? ab.abilityLevel ?? ab.lvlAbility ?? ab.skillLevel ?? 1) + d.delta;

      if (typeof renderVillageChief === 'function') renderVillageChief();
...
function autoClickTick() {
  ...
  if (villageChief.level >= CHIEF_MAX_LEVEL) {
    ...
    addHeroExp(partner, 1, PARTNER_MAX_LEVEL);
...
function renderVillageChief() {
  ...
  // Zoom button removed as requested
  avatarWrap.appendChild(img);

  // Add persistent download button
  const chiefDownload = document.createElement('div');
  chiefDownload.className = 'persistent-download-icon';
  chiefDownload.textContent = '🡇';
  chiefDownload.classList.toggle('disabled', !villageChief.avatar);
  chiefDownload.title = 'Download Village Chief image';
  chiefDownload.setAttribute('aria-label', 'Download Village Chief image');
  chiefDownload.onclick = e => {
    e.stopPropagation();
    if (chiefDownload.classList.contains('disabled')) return;
    downloadImage(villageChief.avatar);
  };
  ...
  avatarCol.appendChild(musicSlot);
  ...
  bossStatsBtn.style.marginTop = "4px";
  ...
  inventoryBtn.style.marginTop = "4px";
  ...
  info.appendChild(inventoryBtn);



  const familiarsBtn = document.createElement("button");

  familiarsBtn.textContent = "Familiars";

  familiarsBtn.className = "btn btn-green";

  familiarsBtn.style.marginTop = "4px";

  familiarsBtn.onclick = () => {

      if (!openChiefFamiliars) {

        openChiefHabilities = false;

        openChiefPopulation = false;

        chiefFamiliarSort = 'number';

        chiefFamiliarPage = 1;

      }

    openChiefFamiliars = !openChiefFamiliars;

    renderVillageChief();

  };

  info.appendChild(familiarsBtn);
  ...
  const populationBtn = document.createElement("button");
    ...
    populationBtn.className = "btn btn-green";

    populationBtn.style.marginTop = "4px";
    ...
    populationBtn.onclick = () => {
      ...
      openChiefPopulation = !openChiefPopulation;

      renderVillageChief();

    };

    info.appendChild(populationBtn);
  ...
  habilitiesBtn.style.marginTop = "4px";
  ...
  info.appendChild(habilitiesBtn);
  ...
  if (!readOnly) {
    ...
    partnerImg.title = "Edit Image (500x500px recommended)";

    partnerImg.onclick = () => {

      if (lockPartnerImage) return;
  ...
  if (!readOnly) {
    ...
    nameDiv.onclick = () => {
      ...
      openEditModal('Partner Name', partner.name, val => {
  ...
  partnerSlot.appendChild(nameDiv);

  partnerWrap.appendChild(partnerImg);

  // Add persistent download button
  const partnerDownload = document.createElement('div');
  partnerDownload.className = 'persistent-download-icon';
  partnerDownload.textContent = '🡇';
  partnerDownload.classList.toggle('disabled', !partner.img);
  partnerDownload.title = 'Download Partner image';
  partnerDownload.setAttribute('aria-label', 'Download Partner image');
  partnerDownload.onclick = e => {
    e.stopPropagation();
    if (partnerDownload.classList.contains('disabled')) return;
  ...
  if (!readOnly) {
    ...
    addMoveArrows(partnerWrap, partnerImg, partner, 'imgOffset', false, 'imgOffsetX');
  ...
  const pExpNeeded = expNeededForLevel(partner.level, PARTNER_MAX_LEVEL);

  const pExp = partner.exp ?? 0;

  const pExpText =

    partner.level >= PARTNER_MAX_LEVEL

      ? `Exp: ${pExp}`

      : `Exp: ${pExp}/${pExpNeeded}`;

  lvl.textContent = `Level: ${partner.level}/${PARTNER_MAX_LEVEL} ${pExpText}`;

  lvl.className = 'partner-level';

  partnerSlot.appendChild(lvl);
  ...
  unlockBtn.textContent = "FightIntruders";

  unlockBtn.className = "btn btn-lightred";

  unlockBtn.title = "Use Chief and Partner Abilities to collect Gold";

  unlockBtn.style.flex = "1";

  unlockBtn.style.width = "100%";

  unlockBtn.style.fontSize = "0.8em";

  unlockBtn.id = "unlock-partner-btn";

  unlockBtn.onclick = () => {

    if (currentChiefExtra === "FightIntruders") {
      ...
      showChiefExtra("FightIntruders");

    }

  };

  partnerRow.appendChild(unlockBtn);

  if (currentView === "profiles") {

    partnerRow.style.gap = "0";

    partnerRow.style.justifyContent = "center";

    partnerAbBtn.style.flex = "none";

    partnerAbBtn.style.width = "100%";

    unlockBtn.style.flex = "none";
  ...
  pInvBtn.onclick = () => { openPartnerInventory = !openPartnerInventory; renderVillageChief(); };
  ...
  const partnerExtraRow = document.createElement("div");

  partnerExtraRow.style.display = "flex";

  partnerExtraRow.style.gap = "4px";

  partnerExtraRow.style.marginTop = "4px";

  partnerExtraRow.style.width = "100%";
  ...
  partnerExtraRow.appendChild(pB1);

  partnerExtraRow.appendChild(pB2);

  partnerSlot.appendChild(partnerExtraRow);
  ...
  shopBtn.style.marginBottom = "4px";

  shopBtn.onclick = () => showResourceShop({ container: card });

  info.appendChild(shopBtn);
  ...
  summonBtn.textContent = `HeroSummon (${summonCost} Gold)`;

  summonBtn.style.paddingTop = "4px";

  summonBtn.style.paddingBottom = "4px";
  ...
  const fortuneCol = document.createElement("div");

  fortuneCol.className = "chief-action-col";

  fortuneCol.style.gap = "4px";

  fortuneCol.style.marginLeft = "0";



  fortuneCol.appendChild(summonBtn);
  ...
  promoteBtn.className = "btn btn-blue";
  ...
  fortuneCol.appendChild(promoteBtn);



  const familiarBtn = document.createElement("button");

  familiarBtn.id = "familiar-btn";

  const familiarCost = 1500 * ((villageChief.unlockedFamiliars || unlockedFamiliars) + 1);

  familiarBtn.textContent = `SummonFamiliar (${familiarCost} Gold)`;

  familiarBtn.className = "btn btn-blue";

  familiarBtn.onclick = () => {

    openConfirm({

      message: `Spend ${familiarCost} Gold to summon a familiar?`,

      container,

      onConfirm: summonFamiliar,

    });

  };

  familiarBtn.disabled = state.money < familiarCost || (villageChief.unlockedFamiliars || unlockedFamiliars) >= FAMILIAR_COUNT;

  if (familiarBtn.disabled && state.money < familiarCost) familiarBtn.title = "Not enough Gold";

  fortuneCol.appendChild(familiarBtn);
  ...
  abilityBtn.textContent = `LearnAbility (${abilityCost} Gold)`;
  ...
  fortuneCol.appendChild(abilityBtn);



  const partnerAbilityBtn = document.createElement('button');

  partnerAbilityBtn.id = 'partner-ability-btn';

  const pAbilityCost = 2000 * (unlockedPartnerAbilities + 1);

  partnerAbilityBtn.textContent = `UnlockPartnerAbility (${pAbilityCost} Gold)`;

  partnerAbilityBtn.className = 'btn btn-blue';

  partnerAbilityBtn.style.fontSize = '0.9em';

  partnerAbilityBtn.onclick = () => {

    openConfirm({

      message: `Spend ${pAbilityCost} Gold to unlock partner ability?`,

      container,

      onConfirm: unlockPartnerAbility,

    });

  };

  partnerAbilityBtn.disabled = state.money < pAbilityCost || unlockedPartnerAbilities >= PARTNER_ABILITY_COUNT;

  if (partnerAbilityBtn.disabled && state.money < pAbilityCost) partnerAbilityBtn.title = 'Not enough Gold';

  fortuneCol.appendChild(partnerAbilityBtn);



  const missionCol = document.createElement("div");

  missionCol.className = "chief-action-col";

  missionCol.style.gap = "4px";

  missionCol.style.marginLeft = "4px";

  const checklistBtn = document.createElement("button");

  checklistBtn.textContent = "LifeMissions";

  checklistBtn.className = "btn btn-celeste";

  checklistBtn.style.paddingLeft = "10px";

  checklistBtn.style.paddingRight = "10px";

  checklistBtn.onclick = () => {

    if (currentChiefExtra === "Life Missions") {
      ...
      showChiefExtra("Life Missions");

    }

  };

  const habitsBtn = document.createElement("button");

  const todayStr = `${new Date().getDate()}/${new Date().getMonth() + 1}/${new Date().getFullYear()}`;

  habitsBtn.innerHTML = `HabitsCalendar<br>(${todayStr})`;

  habitsBtn.className = "btn btn-celeste";

  habitsBtn.style.paddingLeft = "10px";

  habitsBtn.style.paddingRight = "10px";
  ...
  habitsBtn.onclick = () => {
    ...
    if (currentChiefExtra === "Habits") {
      ...
      showChiefExtra("Habits");

    }

  };

  missionCol.appendChild(checklistBtn);

  missionCol.appendChild(habitsBtn);
  ...
  projectsBtn.className = "btn btn-celeste";

  projectsBtn.style.paddingLeft = "10px";

  projectsBtn.style.paddingRight = "10px";

  projectsBtn.title = "Increase PAbility level";

  projectsBtn.onclick = () => {

    if (currentChiefExtra === "Projects") {
      ...
      showChiefExtra("Projects");

    }

  };

  missionCol.appendChild(projectsBtn);



  const silenceBtn = document.createElement("button");

  silenceBtn.textContent = "SilenceTemple";

  silenceBtn.className = "btn btn-celeste";

  silenceBtn.title = "Increase Familiar Level";

  silenceBtn.onclick = showSilenceTempleModal;

  const pomoBtn = document.createElement("button");

  pomoBtn.textContent = "PomodoroTower";

  pomoBtn.className = "btn btn-celeste";

  pomoBtn.title = "Increase Ability Level";

  pomoBtn.onclick = showPomodoroTowerModal;

  missionCol.appendChild(silenceBtn);

  missionCol.appendChild(pomoBtn);
  ...
  missionCol.style.paddingLeft = "0";



    if(openChiefFamiliars || openChiefPopulation || openChiefHabilities || openPartnerAbilities || openChiefInventory || openBossStats || openPartnerStats || openHeroesManagement){

    fortuneCol.style.marginRight = "5px";

    missionCol.style.marginLeft = "5px";

  } else {

    fortuneCol.style.marginRight = "0";
  ...
  card.appendChild(fortuneCol);

  card.appendChild(missionCol);
  ...
  famList.forEach((fam, idx) => {
    ...
    // Add persistent download button
    const famDownload = document.createElement('div');
    famDownload.className = 'persistent-download-icon';
    famDownload.textContent = '🡇';
    famDownload.classList.toggle('disabled', !fam.img);
    famDownload.title = 'Download Familiar image';
    famDownload.setAttribute('aria-label', 'Download Familiar image');
    famDownload.onclick = e => {
      e.stopPropagation();
      if (famDownload.classList.contains('disabled')) return;
  ...
  // Add pagination container for familiars
  const famPagination = document.createElement('div');
  famPagination.id = 'familiar-pagination';
  famPagination.className = 'pagination';
  famDiv.appendChild(famPagination);

  // Advanced Pagination for familiars
  if (famPages > 1) {
    createPaginationBar({
      currentPage: chiefFamiliarPage,
      totalPages: famPages,
      onPageChange: (page) => {
        chiefFamiliarPage = page;
        renderVillageChief();
      },
      container: famPagination,
      showGoToPage: true
    });
  }
    ...
    popClose.style.paddingTop = "8px";
    popClose.style.paddingBottom = "25px";
    ...
    citizenBtn.textContent = "SummonCitizen (100 Gold)";
    citizenBtn.className = "btn btn-blue";
    citizenBtn.onclick = () => {
      openConfirm({
        message: 'Spend 100 Gold to summon a citizen?',
        container,
        onConfirm: summonCitizen,
      });
    };
    citizenBtn.disabled = citizens >= state.terrain * 50 || state.money < 100;
    if (citizenBtn.disabled && state.money < 100) citizenBtn.title = "Not enough Gold";
    summonButtonsRow.appendChild(citizenBtn);

    const soldierBtn = document.createElement("button");
    soldierBtn.id = "soldier-btn";
    soldierBtn.textContent = "SummonSoldier (200 Gold)";
    soldierBtn.className = "btn btn-blue";
    soldierBtn.onclick = () => {
      openConfirm({
        message: 'Spend 200 Gold to summon a soldier?',
        container,
        onConfirm: summonSoldier,
      });
    };
    ...
    // Agregar contadores debajo de los botones
    const countersRow = document.createElement("div");
    countersRow.style.display = "flex";
    countersRow.style.gap = "20px";
    countersRow.style.justifyContent = "center";
    countersRow.style.textAlign = "center";
    countersRow.style.width = "100%";
    countersRow.style.flexWrap = "wrap";
    ...
    // Contadores adicionales con el mismo tamaño
    const familiarsCounter = document.createElement("div");
    familiarsCounter.id = "familiars-display";
    const unlockedFamiliars = villageChief.unlockedFamiliars || 0;
    familiarsCounter.textContent = `Familiars: ${unlockedFamiliars}/100`;

    const heroesCounter = document.createElement("div");
    heroesCounter.id = "heroes-total-display";
    heroesCounter.textContent = `Heroes: ${state.heroes.length}`;

    const villainsCounter = document.createElement("div");
    villainsCounter.id = "villains-total-display";
    villainsCounter.textContent = `Villains: ${villains.length}`;

    const petsCounter = document.createElement("div");
    petsCounter.id = "pets-total-display";
    const totalPets = state.heroes.filter(h => h.pet).length;
    petsCounter.textContent = `Pets: ${totalPets}`;

    const elitesCounter = document.createElement("div");
    elitesCounter.id = "elites-display";
    elitesCounter.textContent = `Elites: 0`;

    const specialCitizensCounter = document.createElement("div");
    specialCitizensCounter.id = "special-citizens-display";
    specialCitizensCounter.textContent = `SpecialCitizens: 0`;

    const specialSoldiersCounter = document.createElement("div");
    specialSoldiersCounter.id = "special-soldiers-display";
    specialSoldiersCounter.textContent = `SpecialSoldiers: 0`;

    countersRow.appendChild(cit);
    countersRow.appendChild(sol);
    countersRow.appendChild(familiarsCounter);
    countersRow.appendChild(heroesCounter);
    countersRow.appendChild(villainsCounter);
    countersRow.appendChild(petsCounter);
    countersRow.appendChild(elitesCounter);
    countersRow.appendChild(specialCitizensCounter);
    countersRow.appendChild(specialSoldiersCounter);

    popCard.appendChild(countersRow);
    ...
    popCard.appendChild(subtitlesContainer);
  ...
  habList.forEach((hab, idx) => {
    ...
    // Add persistent download button
    const abDownload = document.createElement('div');
    abDownload.className = 'persistent-download-icon';
    abDownload.textContent = '🡇';
    abDownload.classList.toggle('disabled', !imgSrc);
    abDownload.title = 'Download Ability image';
    abDownload.setAttribute('aria-label', 'Download Ability image');
    abDownload.onclick = e => {
      e.stopPropagation();
      if (abDownload.classList.contains('disabled')) return;
      downloadImage(imgSrc);
    };
    imgDiv.appendChild(abDownload);
  ...
  // Add pagination container for abilities
  const habPagination = document.createElement('div');
  habPagination.id = 'ability-pagination';
  habPagination.className = 'pagination';
  habDiv.appendChild(habPagination);

  // Advanced Pagination for abilities
  if (habPages > 1) {
    createPaginationBar({
      currentPage: chiefHabilityPage,
      totalPages: habPages,
      onPageChange: (page) => {
        chiefHabilityPage = page;
        renderVillageChief();
      },
      container: habPagination,
      showGoToPage: true
    });
  }
  ...
  partList.forEach((ab,idx)=>{
    ...
    // Add persistent download button
    const pAbDownload = document.createElement('div');
    pAbDownload.className = 'persistent-download-icon';
    pAbDownload.textContent = '🡇';
    pAbDownload.classList.toggle('disabled', !imgSrcP);
    pAbDownload.title = 'Download Partner Ability image';
    pAbDownload.setAttribute('aria-label', 'Download Partner Ability image');
    pAbDownload.onclick = e => {
      e.stopPropagation();
      if (pAbDownload.classList.contains('disabled')) return;
      downloadImage(imgSrcP);
    };
    imgDiv.appendChild(pAbDownload);
  ...
  // Add pagination container for p_abilities
  const partPagination = document.createElement('div');
  partPagination.id = 'pability-pagination';
  partPagination.className = 'pagination';
  partnerDiv.appendChild(partPagination);

  // Advanced Pagination for p_abilities
  if (partPages > 1) {
    createPaginationBar({
      currentPage: partnerAbilityPage,
      totalPages: partPages,
      onPageChange: (page) => {
        partnerAbilityPage = page;
        renderVillageChief();
      },
      container: partPagination,
      showGoToPage: true
    });
  }
...
let sywBtnChange, sywBtnAdd, sywBtnRemove, sywCurrentEl, sywDurationEl, sywSeekEl, sywCard;
...
function initChiefMusicPlayer(card){
  ...
  if (!newMute || !newPrev || !newPlay || !newNext || !newTitle || !newChange || !newAdd || !newRemove || !newCurrent || !newDuration || !newSeek) return;
  ...
  if (label === "Autoclick enabled") {
    ...
    iframe.onload = () => {
      ...
      try {
        ...
        const partnerAbilities = (villageChief.partnerAbilities || [])
        ...
        iframe.contentWindow.postMessage({
          ...
          partner: {
            ...
            unlockedPartnerAbilities: villageChief.unlockedPartnerAbilities ?? unlockedPartnerAbilities,

            abilities: partnerAbilities
    ...
    iframe.onload = () => {
      ...
      iframe.contentWindow.postMessage({
        ...
        chief: {
          ...
          unlockedHabilities: villageChief.unlockedHabilities ?? unlockedHabilities,
...
function showDailyTribute() {
  ...
  const familiarCount = villageChief.unlockedFamiliars || unlockedFamiliars;
...
function showView(view = "home") {
  ...
  if (sections.heroes) {
    ...
    const hideHeroes = ["events", "missions", "games", "village", "tutorial", "settings"].includes(view);
...
export function showSilenceTempleModal() {
  ...
  for (let i = 0; i < (villageChief.unlockedFamiliars || unlockedFamiliars); i++) {
  ...
  [
    ...
    ['446','🫁 4-4-6 Relaxing (Inhale 4s, hold 4s, exhale 6s)'],

    ['kapalabhati','🔥 Kapalabhati Energizing (Rapid breathing with forced exhalations)'],

    ['nadi','🌬️ Nadi Shodhana / Nadi Balancing (Alternate nostril breathing)']
  ...
  const techBenefits = {
    ...
    '446': [
      ...
      'Improves sleep and general relaxation'
    ...
    'kapalabhati': [
      ...
      'Cleanses the respiratory tract and awakens the body'
    ...
    'nadi': [
      ...
      'Harmonizes the energy flow (prana)'
  ...
  ok.onclick = () => {
    ...
    renderSilenceTempleCard(currentSilenceFam, tech);
...
function renderSilenceTempleCard(fam, tech = '446') {
  ...
  url.searchParams.set('name', fam.name);

  url.searchParams.set('level', fam.level || 1);
  ...
  iframe.onload = () => {
    ...
    iframe.contentWindow.postMessage({
      ...
      img: resolveSrc(fam.img || EMPTY_SRC)
  ...
  silenceMsgHandler = e => {
    ...
    if (e.data && e.data.type === 'silenceTempleComplete' && currentSilenceFam) {

      currentSilenceFam.level = (currentSilenceFam.level || 1) + 1;
      ...
      showAlert(`${currentSilenceFam.name} leveled up! You also won 2000Gold!`, { container: vc });
...
function renderVillains() {
  ...
  pageList.forEach(vil => {
    ...
    statsBtn.onclick = () => {
      const statsEl = document.getElementById(`vil-stats-${vil.id}`);
      if (statsEl.classList.contains("expand-row")) {
        statsEl.classList.remove("expand-row");
      } else {
        statsEl.classList.add("expand-row");
      }
    };
    ...
    closeBtn.className = "close-btn close-small";
  ...
  // Advanced Pagination
  if (pagination && villainPages > 1) {
    createPaginationBar({
      currentPage: currentVillainPage,
      totalPages: villainPages,
      onPageChange: (page) => {
        currentVillainPage = page;
        renderVillains();
      },
      container: pagination,
      showGoToPage: true
...
function renderTutorial(){
  ...
  if (infoCard) {
    ...
    importantNotes.innerHTML = `
      <h2>1. Important Notes</h2>
      <p><strong>1.1. Save & Music Folders:</strong> Save files are stored in ...\\documents\\SummonYourWillSaves; music is stored in ...\\SummonYourWillMusic.</p>
    `;
    infoCard.appendChild(importantNotes);

    // 2. Characters
    const characters = document.createElement('div');
    characters.innerHTML = `
      <h2>2. Characters</h2>
      <h3>2.1 Village Chief</h3>
      <p><strong>Role:</strong> The leader of your village.</p>
      <p><strong>Has:</strong> Abilities, Stats, and an Inventory.</p>
      
      <h3>2.2 Partner</h3>
      <p><strong>Role:</strong> Your co-lead who grows through project work.</p>
      <p><strong>Has:</strong> Abilities, Stats, and an Inventory.</p>
      <p><strong>Ability leveling (via the Projects minigame):</strong></p>
      <ul>
        <li>Earn 1 Project Point per real-time minute of focused work.</li>
        <li>30 Project Points = +1 level to a chosen Partner ability.</li>
        <li>Finishing a project grants +5 levels to one Partner ability.</li>
      </ul>
      
      <h3>2.3 Familiars</h3>
      <p><strong>Role:</strong> Spiritual companions.</p>
      <p><strong>Limit:</strong> Up to 100 total.</p>
      <p><strong>Unlocking:</strong> Spend Gold.</p>
      <p><strong>Leveling (via Silence Temple):</strong> Meditate 2 minutes to level up the selected Familiar.</p>
      
      <h3>2.4 Heroes (Main Characters)</h3>
      <p><strong>Role:</strong> Primary workforce and adventurers.</p>
      <p><strong>Resource generation:</strong> Auto-clicking, Work / Farm / Mine / Chop, Missions.</p>
      <p><strong>Village building:</strong> Help construct and upgrade structures.</p>
      <p><strong>Each Hero has:</strong> Stats, Inventory, a Pet slot, and two Abilities.</p>
      
      <h3>Key Structures & Effects</h3>
      <ul>
        <li><strong>Terrain:</strong> Increases max Houses (each Terrain level adds +5 Houses).</li>
        <li><strong>Houses:</strong> Increase max Heroes (1 Hero per House).</li>
        <li><strong>Pet Sanctuary:</strong> Increases max Pets.</li>
        <li><strong>Tower:</strong> Increases max Villains you can hold.</li>
        <li><strong>Food/Wood/Stone Storage:</strong> Raise caps for Food, Wood, Stone.</li>
        <li><strong>Training Grounds:</strong> Raise max attainable stats for Heroes.</li>
        <li><strong>Dungeon:</strong> Generates daily Gold via Daily Tribute.</li>
        <li><strong>Castle:</strong> Raises level caps for Village Chief, Heroes, Partner.</li>
      </ul>
      
      <h3>2.5 Pets</h3>
      <p><strong>Role:</strong> Assign to Heroes to enhance performance.</p>
    `;
    infoCard.appendChild(characters);

    // 3. Features
    const features = document.createElement('div');
    features.innerHTML = `
      <h2>3. Features</h2>
      <h3>3.1 Silence Temple</h3>
      <p><strong>Modes:</strong></p>
      <ul>
        <li><strong>4-4-6 Relaxing:</strong> Inhale 4s, hold 4s, exhale 6s.</li>
        <li><strong>Kapalabhati Energizing:</strong> Rapid breathing with forceful exhalations.</li>
        <li><strong>Nadi Balancing:</strong> Alternate-nostril breathing.</li>
      </ul>
      <p><strong>Rewards:</strong> Upgrades Familiar level after 2 minutes; also gives Gold.</p>
      
      <h3>3.2 Pomodoro Tower</h3>
      <p><strong>Technique:</strong> Focus cycles of 25 or 45 minutes.</p>
      <p><strong>Rewards:</strong> Upgrades Village Chief ability level; also gives Gold.</p>
      
      <h3>3.3 Projects</h3>
      <p>Manage your real-life projects.</p>
      <p><strong>Rewards:</strong> Upgrades Partner abilities (see Partner rules above).</p>
      
      <h3>3.4 Habits Calendar</h3>
      <p>Daily habits → Chief stats:</p>
      <p>Exercise → Strength, Study → Intelligence, Meditation → Mana, etc.</p>
      <p><strong>Rewards:</strong> Upgrades Village Chief stats; also gives Gold.</p>
      
      <h3>3.5 Life Missions</h3>
      <p>Daily tasks with difficulty tiers:</p>
      <ul>
        <li><strong>Easy:</strong> 400 Gold</li>
        <li><strong>Normal:</strong> 1000 Gold</li>
        <li><strong>Hard:</strong> 2500 Gold</li>
      </ul>
      
      <h3>3.6 Custom Music Player</h3>
      <p>Personalized music player for your sessions.</p>
      
      <h3>3.7 Hero Orders</h3>
      <p>Partner issues global orders to all Heroes: Rest, Work, Auto-click.</p>
      
      <h3>3.8 Export / Import / Reset</h3>
      <ul>
        <li><strong>Export:</strong> Save current game data to .json for use later.</li>
        <li><strong>Import:</strong> Load a saved .json game.</li>
        <li><strong>Reset:</strong> Start fresh from level 0 with default data.</li>
      </ul>
      
      <h3>3.9 SaveAllImages (Setting)</h3>
      <p>When enabled: Saves all app images to ..\\images\\SummonYourWillImages.</p>
    `;
    infoCard.appendChild(features);

    // 4. Minigames
    const minigames = document.createElement('div');
    minigames.innerHTML = `
      <h2>4. Minigames</h2>
      <h3>4.1 Fight Intruders</h3>
      <p>Use Village Chief and Partner abilities to repel intruders and earn Gold.</p>
      
      <h3>4.2 Giant Boss</h3>
      <p>6 Heroes fight a giant monster that gets stronger after each defeat.</p>
      
      <h3>4.3 Enemy Encounters</h3>
      <p>Capture-style minigame that adds Villains to the village.</p>
      
      <h3>4.4 Pet Exploration</h3>
      <p>Each Pet can collect Gold once per day with the Village Chief and Partner.</p>
      
      <h3>4.5 Chief Survival</h3>
      <p>Earn 1000 Gold for each level the Chief survives.</p>
      
      <h3>4.6 Tower Defense (Coming Soon)</h3>
      <p>12 Heroes, each must be at 100% energy.</p>
      <p><strong>Rewards:</strong> +100 EXP and +1000 Gold per level reached.</p>
      
      <h3>4.7 Fortune Wheel</h3>
      <p>Daily wheel to earn Gold.</p>
    `;
    infoCard.appendChild(minigames);
...
function renderGames() {
  ...
  if (buttonsCard && detailCard) {
    ...
    for (let i = 1; i <= Object.keys(gameNames).length; i++) {
      ...
      } else if (label === "TowerDefense") {

        btn.textContent = label;
        btn.title = "-100% energy +100exp +1000Gold*Lvl";
      ...
      btn.onclick = () => {
        ...
        if (label === "TowerDefense") {
          ...
          // Verificar que haya al menos 12 héroes disponibles

          const availableHeroes = state.heroes.filter(h => !isBusy(h));

          if (availableHeroes.length < 12) {

            showAlert(`Necesitas al menos 12 héroes disponibles. Tienes ${availableHeroes.length} disponibles.`, { container: detailCard });
          ...
          // Mostrar selector de héroes para TowerDefense

          chooseHeroesForTowerDefense(detailCard, gamesSection);
            ...
            iframe.onload = () => {
              ...
              if (label === "PetExploration") {
                ...
                iframe.contentWindow.postMessage({
                  ...
                  familiar: selectedFamiliarAvatar,
        ...
        const loadAndLaunch = hero => {
          ...
          if (label === "PetExploration") {
            ...
            selectedFamiliarAvatar = resolveSrc(partner.img || partner.photoUrl || partner.imageUrl || partner.avatar || EMPTY_SRC);
      ...
      if (label === "GiantBoss") {
        ...
        info.textContent = "3 chances left";
...
function renderUniversesSection() {
  ...
  // Remover elementos existentes si existen
  let universesCard = document.getElementById("universes-card");
  if (universesCard) universesCard.remove();
  const oldTitle = document.getElementById("universes-title");
  if (oldTitle) oldTitle.remove();

  // Título similar a "MY GAMES"
  const title = document.createElement("h1");
  title.id = "universes-title";
  title.textContent = "SUMMONYOURWILL UNIVERSES (OTHER GAMES)";
  ...
  universesCard = document.createElement("div");
  universesCard.id = "universes-card";
  universesCard.className = "card gold-border";
  universesCard.style.width = "100%";
  universesCard.style.minHeight = "120px";

  gamesSection.appendChild(title);
  gamesSection.appendChild(universesCard);
}
...
function renderLocationsSection() {
  ...
  // Remover elementos existentes si existen
  let locationsCard = document.getElementById("locations-card");
  if (locationsCard) locationsCard.remove();
  const oldLocTitle = document.getElementById("locations-title");
  if (oldLocTitle) oldLocTitle.remove();

  // Título similar a "MY GAMES"
  const locTitle = document.createElement("h1");
  locTitle.id = "locations-title";
  locTitle.textContent = "LOCATIONS";
  locTitle.style.fontSize = "1.5em";
  locTitle.style.fontWeight = "normal";
  locTitle.style.marginTop = "20px";
  locTitle.style.marginBottom = "10px";
  locTitle.style.fontFamily = "serif";

  // Card con borde dorado
  locationsCard = document.createElement("div");
  locationsCard.id = "locations-card";
  locationsCard.className = "card gold-border";
  locationsCard.style.width = "100%";
  locationsCard.style.minHeight = "120px";

  gamesSection.appendChild(locTitle);
  gamesSection.appendChild(locationsCard);
}
...
function summonCitizen() {
  ...
  const max = state.terrain * 50;

  if (citizens >= max || state.money < 100) return;
...
function summonSoldier() {
  ...
  if (soldiers >= max || state.money < 200) return;
...
function summonFamiliar() {
  ...
  const currentUnlocked = villageChief.unlockedFamiliars || unlockedFamiliars;
  const cost = 1500 * (currentUnlocked + 1);

  if (state.money < cost || currentUnlocked >= FAMILIAR_COUNT) return;

  state.money -= cost;

  unlockedFamiliars = currentUnlocked + 1;

  villageChief.unlockedFamiliars = unlockedFamiliars;
...
async function resetGameCompletely() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (!partida0Data || typeof partida0Data !== 'object' || partida0Data.version === undefined) {
        showAlert('Error: partida0.json does not have the correct structure.');
        return;
      }

      // Limpiar localStorage antes de importar
      localStorage.clear();
      sessionStorage.clear();

      // Establecer el nuevo estado del juego
      if (ipcRenderer) {
        ipcRenderer.send('set-game-state', partida0Data);
      } else {
        setItem('gameState', partida0Data);
      }

      // Recargar el juego con los nuevos datos
      await loadGame();
      ...
      const container = getImportAlertContainer();
      showAlert('Reset successful. The game has been reset with partida0.json data.', container ? { container } : {});

    } else {
      showAlert('Error: Could not load partida0.json');
    }

  } catch (error) {
    console.error('Error al cargar partida0.json:', error);
    showAlert("Error during reset: " + error.message);
  }
...
function getEligibleHeroes() {
  ...
  return (state.heroes || []).filter(h => {
    ...
    // Verificar que no esté asignado en Special Builder Assignment
    const isAssignedInSpecialBuilder = state.specialBuilderSlots?.some(slot => 
      slot.status !== 'idle' && slot.assignedHeroId === h.id
    ) || false;

    return hasEnergy && !isHeroBusy(h.id) && !isAssignedInSpecialBuilder;
...
async function init() {
  ...
  const showPerfHUD = (localStorage.getItem('showPerfHUD') === '1');

  if (showPerfHUD) {

    await initStatsMonitor();

    wireStatsVisibility(ipcRenderer);

  } else {

    try { localStorage.setItem('statsHidden','true'); } catch {}
  ...
  document.getElementById("sort-level-btn").onclick = () => {
...
const api = {
  ...
  renderDailyMissions: () => {}, // Función vacía para ocultar Daily Missions
...
function createPaginationBar(options) {
  const {
    currentPage,
    totalPages,
    onPageChange,
  ...
  paginationDiv.setAttribute('aria-label', 'Paginación de listado');
  ...
  if (showGoToPage && totalPages > 5) {
    const goToDiv = document.createElement('div');
    goToDiv.className = 'pagination-goto';
    ...
    goToDiv.appendChild(label);
    goToDiv.appendChild(input);
    paginationDiv.appendChild(goToDiv);
  }
  ...
  addKeyboardNavigation(paginationDiv, currentPage, totalPages, onPageChange);
}

function createPaginationButton(text, type, disabled, onClick, isActive = false) {
  const button = document.createElement('button');
  button.textContent = text;
  button.className = `pagination-btn pagination-btn-${type}`;
...
function addKeyboardNavigation(container, currentPage, totalPages, onPageChange) {
