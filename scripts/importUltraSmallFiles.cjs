// scripts/importUltraSmallFiles.cjs
...
async function importUltraSmallFile(filePath, upsertFunction, collectionName) {
  try {
...
(async () => {
  await connectMongo();
  try {
    // Importar partner ultra pequeño (con _id único y partnerAbilities)
    try {
      await importUltraSmallFile(
    ...
    // Importar villagechief ultra pequeño (sin abilities.habilities, con abilities renombrado)
    try {
      await importUltraSmallFile(
  ...
  console.log('🎉 Importación de archivos ultra pequeños completada');
})();
